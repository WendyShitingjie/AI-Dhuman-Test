...
class AITestExecutor:
    ...
    def _execute_test_steps(self, test_case: Dict, context: ExecutionContext):
        ...
        for i, step in enumerate(test_steps):
            ...
            # 检查步骤是否只有断言（没有 action）
            has_action = 'action' in step
            has_assertions = 'assertions' in step and step['assertions']

            # 处理轮询等待时间（每隔 poll_interval 秒执行断言，直到超时）
            wait_seconds = step.get('wait', 0)
            poll_interval = step.get('poll_interval', 20)

            # 判断是否需要轮询：有等待时间且有断言
            needs_polling = wait_seconds > 0 and has_assertions and not has_action
            ...
            try:
                ...
                if has_action:
                    ...
                    # 没有 action 且没有 intent_parser，检查是否有断言
                    if not has_assertions:
                        raise ValueError(f"步骤缺少 'action' 字段，且没有 intent_parser 可用: {step.get('intent')}")
                    item_detail['status'] = 'SUCCESS'
                    print(f"      (无 action，仅执行断言)")

                # 执行步骤级别的断言
                if not step_failed and has_assertions:
                    step_assertions = step.get('assertions', [])
                    
                    if needs_polling:
                        print(f"\n      🔄 开始轮询等待 (总超时: {wait_seconds}s, 间隔: {poll_interval}s)")
                        import time
                        elapsed = 0
                        poll_count = 0
                        all_passed = False
                        last_poll_assertions = []
                        
                        while elapsed < wait_seconds:
                            poll_count += 1
                            print(f"\n      🔍 轮询第 {poll_count} 次 (已等待 {elapsed}s)...")
                            
                            poll_passed = True
                            current_poll_assertions = []
                            for j, assertion in enumerate(step.get('assertions', [])):
                                assertion_result = self._execute_single_assertion(
                                    assertion, context, j+1
                                )
                                current_poll_assertions.append(assertion_result)
                            ...
                            if poll_passed:
                                all_passed = True
                                print(f"      ✅ 所有断言通过，轮询成功!")
                                for a in current_poll_assertions:
                                    self.execution_details['assertions'].append(a)
                                break
                            else:
                                last_poll_assertions = current_poll_assertions
                            
                            time.sleep(poll_interval)
                            elapsed += poll_interval
                        
                        if not all_passed:
                            for a in last_poll_assertions:
                                self.execution_details['assertions'].append(a)
                            step_failed = True
                            print(f"      ⛔ 轮询超时 ({wait_seconds}s)，断言未通过")
                    else:
                        # 非轮询模式：直接执行一次断言
    ...
    def _generate_test_result(self, test_case: Dict, context: ExecutionContext,
        ...
        # 检查是否有步骤失败
        steps = self.execution_details.get('test_steps', [])
        any_step_failed = any(s.get('status') == 'FAIL' for s in steps)

        # 检查是否有前置条件失败
        preconditions = self.execution_details.get('preconditions', [])
        any_precondition_failed = any(p.get('status') == 'FAIL' for p in preconditions)

        # 判断是否所有断言都通过
        all_assertions_passed = all(a.get('status') == 'PASS' for a in all_assertions) if all_assertions else True

        # 只有当前置条件、步骤、断言都通过时，测试才通过
        test_passed = not any_step_failed and not any_precondition_failed and all_assertions_passed
